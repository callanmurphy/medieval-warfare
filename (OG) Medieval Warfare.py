...
def menu():
    ...
    instructionText = font1.render("Use the 'w' and 's' keys to scroll through the menu and 'enter' to select", False, (255,255,255))
...
def main():
        ...
        backgroundImage = pygame.image.load("Background.gif").convert() #.convert() reduces a lot of lag by converting pixel format
        ...
        armourImage = [pygame.image.load("Armour0.gif"), pygame.image.load("Armour1.gif"), pygame.image.load("Armour2.gif"), pygame.image.load("Armour3.gif")]
        ...
        while True:
                for event in pygame.event.get():
                    if event.type == pygame.KEYDOWN:
                        ...
                        if event.key == pygame.K_b and playerHealth > 0 and cliffSpeaking == False:
                                pygame.display.quit()
                                gold, townies, playerHealth, dead, deadImages, firstBattle, playerArmour = battle(characters, townies, goldImage, goldIcon, goldText, healthImage, playerHealth, health, healthText, armourImage, playerArmour, armour, armourText, player, playerImage, friendlyImages, treeImage, gold, dead, deadImages, playerLeft, playerRight, firstBattle)
                                screen = pygame.display.set_mode((1280, 705))
                                playerArmour = 0
                ...
                if player.colliderect(shop):
                    pygame.display.quit()
                    gold, goldText, playerLeft, playerRight, playerHealth, playerArmour = shopping(gold, goldImage, goldIcon, font1, goldText, slot, playerLeft, playerRight, playerHealth, playerArmour)
                ...
                if player.colliderect(hospital):
                    pygame.display.quit()
                    hospitalVisit = medical(player, playerImage, exitLoop, speechFont, dead, deadImages, chargeBars, health, chargeBarsRect, townies, deadCharge, deadCounters, friendlyImages, playerLeft, playerRight, characters, hospitalVisit)
                    screen = pygame.display.set_mode((1280, 705))
                    player.centerx = 770
                    player.centery = 360
                ...
                if speechCounter != 800:
                    ...
                    if speechCounter < 100:
                        ...
                        introSpeech1 = speechFont.render("Press the 'b' key at any time to join us in a battle!", False, (255, 255, 255))
                        screen.blit(introSpeech1,(captain.centerx,captain.centery - 70))
                    ...
                    if townieSpeechCounter != 70:
                        ...
                        if townieSpeechCounter == 1:
                            ...
                            if choice == 1:
                                ...
                                phrase = speechFont.render("I could go for a change of scenery around here...", False, (255, 255, 255))
                            elif choice == 5:
...
def medical(player, playerImage, exitLoop, speechFont,dead, deadImages, chargeBars, health, chargeBarsRect, townies, deadCharge, deadCounters, friendlyImages, playerLeft, playerRight, characters, hospitalVisit):
    '''
    ...
    while True:
                ...
                for i in beds:
                    melonSpeech = speechFont.render("You cannot join the hospital! You must purchase watermelon from the shop to heal yourself", False, (255, 255, 255))
                    if player.colliderect(i):
                ...
                if hospitalVisit == 0:
                    if speechCounter != 900:
                        ...
                        if speechCounter < 100:
                            ...
                            introSpeech1 = speechFont.render("We have 6 beds here for any injured townies.", False, (255, 255, 255))
                            hospitalScreen.blit(introSpeech1,(nurse.centerx,nurse.centery - 75))
                        elif speechCounter < 400:
                            introSpeech1 = speechFont.render("When a townie becomes injured in a battle...", False, (255, 255, 255))
                            hospitalScreen.blit(introSpeech1,(nurse.centerx,nurse.centery - 75))
                        elif speechCounter < 500:
                            introSpeech1 = speechFont.render("they will arrive here and recover over time.", False, (255, 255, 255))
                            hospitalScreen.blit(introSpeech1,(nurse.centerx,nurse.centery - 75))
                        elif speechCounter < 600:
                            ...
                            introSpeech1 = speechFont.render("If you have any questions, I'll be around!", False, (255, 255, 255))
                            hospitalScreen.blit(introSpeech1,(nurse.centerx,nurse.centery - 75))
                    else:
                        nurseSpeaking = False
                        hospitalVisit = 1
...
def battle(characters, friendlies, goldImage, goldIcon, goldText, healthImage, playerHealth, health, healthText, armourImage, playerArmour, armour, armourText, player, playerImage, friendlyImages, treeImage, gold, dead, deadImages, playerLeft, playerRight, firstBattle):
        '''
        ...
        if firstBattle == True:
            font1 = pygame.font.SysFont('Calibri', 30)
            tutorialText1 = font1.render("In a battle, your team must survive and defeat the other team!", False, (255, 255, 255))
            tutorialText2 = font1.render("Kill enemies by running into them, however be careful that they are not in attack mode.", False, (255, 255, 255))
            tutorialText3 = font1.render("Enemies will damage you when in attack mode (left) and will not when in passive mode (right)", False, (255, 255, 255))
        ...
        battleTree = []
        friendlyImages2 = [pygame.image.load("Cliff2.gif"), pygame.image.load("Townie1-2.gif"), pygame.image.load("Townie2-2.gif"), pygame.image.load("Townie3-2.gif"), pygame.image.load("Townie4-2.gif")]
        ...
        while True: #main battle loop
                ...
                if len(enemies) > 0 and team == 1 or len(enemies) - len(soldierNum[1]) > 0 and team == 2 or len(enemies) - len(soldierNum[1]) > 0 and team == 3:
                    if counter == 0:
                    ...
                    if counter == randomCounterEnd:
                            ...
                            while True:
                                    ...
                                    movingEnemy4 = 3
                                    graphic42 = False
                                    if len(enemies) > 1 and team == 1 or len(enemies) - len(soldierNum[1]) > 1 and team == 2 or len(enemies) - len(soldierNum[1]) > 1 and team == 3:
                                        movingEnemy1, graphic12 = randomEnemySelector(soldierNum, team)
                                    if len(enemies) > 2 and team == 1 or len(enemies) - len(soldierNum[1]) > 2 and team == 2 or len(enemies) - len(soldierNum[1]) > 2 and team == 3:
                                        movingEnemy2, graphic22 = randomEnemySelector(soldierNum, team)
                                    if len(enemies) > 3 and team == 1 or len(enemies) - len(soldierNum[1]) > 3 and team == 2 or len(enemies) - len(soldierNum[1]) > 3 and team == 3:
                                        movingEnemy3, graphic42  = randomEnemySelector(soldierNum, team)
                                    if len(enemies) > 5 and team == 1 or len(enemies) - len(soldierNum[1]) > 5 and team == 2 or len(enemies) - len(soldierNum[1]) > 5 and team == 2:
                                    ...
                                    if movingEnemy1 != movingEnemy2 and movingEnemy1 != movingEnemy3 and movingEnemy1 != movingEnemy4 and movingEnemy2 != movingEnemy3 and movingEnemy2 != movingEnemy4 and movingEnemy3 != movingEnemy4:
                                            break
                            if len(enemies) > 5 and team == 1 or len(enemies) - len(soldierNum[1]) > 5 and team == 2 or len(enemies) - len(soldierNum[1]) > 5 and team == 3:
                                movingEnemies = [movingEnemy1, movingEnemy2, movingEnemy3, movingEnemy4]
                            if len(enemies) > 3 and team == 1 or len(enemies) - len(soldierNum[1]) > 3 and team == 2 or len(enemies) - len(soldierNum[1]) > 3 and team == 3:
                                movingEnemies = [movingEnemy1, movingEnemy2, movingEnemy3]
                            elif len(enemies) > 2 and team == 1 or len(enemies) - len(soldierNum[1]) > 2 and team == 2 or len(enemies) - len(soldierNum[1]) > 2 and team == 3:
                                movingEnemies = [movingEnemy1, movingEnemy2]
                            elif len(enemies) > 1 and team == 1 or len(enemies) - len(soldierNum[1]) > 1 and team == 2 or len(enemies) - len(soldierNum[1]) > 1 and team == 3:
                        ...
                        for i in range(0,len(soldrNum)):
                                if willEnemiesMove == 1:
                                    if len(movingEnemies) == 4:
                                        if soldrNum[i] == movingEnemies[0] or soldrNum[i] == movingEnemies[1] or soldrNum[i] == movingEnemies[2] or soldrNum[i] == movingEnemies[3]:
                                                if graphic12 != False:
                                        ...
                                        if soldrNum[i] == movingEnemies[0] or soldrNum[i] == movingEnemies[1] or soldrNum[i] == movingEnemies[2]:
                                                if graphic12 != False:
                                                        battleScreen.blit(soldierChangeImages[team][soldierIndex], soldrNum[i])
                                                if graphic22 != False:
                                                        battleScreen.blit(soldierChangeImages[team][soldierIndex], soldrNum[i])
